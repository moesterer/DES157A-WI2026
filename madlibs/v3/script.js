( function(){
  ...
  function makeMadlib(words){
    ...
    <p>
      The winding pathways lined with scattered <span class="madword">${words[2]}</span> ended in one place: a clearing drowned in <span class="madword">${words[0]}</span> mist.
      <span class="madword">${words[1]}</span> trees scratched at the sky like warnings, and the wind carried the scent of something hungry.
    </p>
    
    <p>
      A woman in a dark cloak <span class="madword">${words[5]}</span> to the center, gripping a <span class="madword">${words[3]}</span> crowned with a <span class="madword">${words[4]}</span> jewel.
      Her ears twitched. She glanced into the woods and whispered <span class="madword">${words[6]}</span>, “They’re coming.”
    </p>
    ...
    <p>
      He laughed, and the sound was a <span class="madword">${words[8]}</span> that didn’t belong in the night.
    ...
    <p>
      She lunged, magic flaring - she began to <span class="madword">${words[7]}</span>, tearing the clearing apart.
      Fire climbed into the air. Shadows stumbled back. A shot rang out and she clutched her heart.
    </p>
    
    <p>
      For the first time since the battle began, Faven faltered. She <span class="madword">${words[9]}</span> to the ground, clutching her <span class="madword">${words[3]}</span>
